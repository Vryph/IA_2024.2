...
public class Game {
  ...
  public List<Move> getLegalPacManMoves() {
    return getLegalPacManMoves(getCurrentGameState());
  }

  public static List<Move> getLegalPacManMoves(GameState state) {
  ...
  public List<Move> getLegalGhostMoves(int ghostIndex) {
    return getLegalGhostMoves(getCurrentGameState(), ghostIndex);
  }

  public static List<Move> getLegalGhostMoves(GameState state, int ghostIndex) {
    List<Move> moves = new ArrayList<Move>();
   ...
   * @return
   */
  public static Set<List<Move>> getLegalCombinedGhostMoves(GameState s) {
   ...
   * @param ghostMoves
   */
  private static void getLegalCombinedGhostMovesHelper(GameState s, List<Move> partialMoves,
  ...
  public static GameState getNextGameState(GameState s, Move pacManMove, List<Move> ghostMoves) {
    ...
    List<GameState> history = s.getHistory();
    ...
    return new GameState(newPacManLocation, newGhostLocations, newDots, ghostMoves, s);
  }
  ...
  public static GameState getNextGameState(GameState s, Move pacManMove) {
    if (isFinal(s))
    ...
    if (newDots.contains(newPacManLocation))
      newDots.remove(newPacManLocation); // eat the dot at that location
    return new GameState(newPacManLocation, s.getGhostLocations(), newDots, s.getPreviousGhostMoves(),
                     s);
  }
   ...
   * @return
   */
  public static GameState getNexGameState(GameState s, List<Move> ghostMoves) {
    List<Location> ghostLocations = s.getGhostLocations();
    ...
    LocationSet newDots = new LocationSet(s.getDotLocations());
    return new GameState(s.getPacManLocation(), newGhostLocations, newDots, ghostMoves, s);
  }
  
  
  public static Set<GameState> getPossibleGhostProjections(GameState s, int distance) {
  ...
  public static Set<GameState> getPossibleGhostProjections(GameState s, Set<Integer> whichGhosts, int distance) {
  ...
  private static void getPossibleGhostProjectionsRecursively(GameState s, List<Set<Location>> possibleLocationsByGhost,
                                                      LinkedList<Location> ghostLocations, Set<GameState> states) {
    if (ghostLocations.size()==s.getGhostLocations().size()) { // we've added locations for all ghosts
      GameState next = new GameState(s.getPacManLocation(), new ArrayList<Location>(ghostLocations), s.getDotLocations(), s.getPreviousGhostMoves(), s);
      states.add(next);
      return;
    }
  ...
  public static Set<Location> getPossibleGhostLocations(GameState s, int ghostIndex, int distance) { 
    Set<Location> result = new HashSet<Location>();
    getPossibleGhostLocationsRecursively(s, ghostIndex, distance, result);
    return result;
  }
  
  private static void getPossibleGhostLocationsRecursively(GameState s, int ghostIndex, int distance, Set<Location> locations) {
    ...
    for (Move m : legalMoves) {
      ...
      GameState next = new GameState(s.getPacManLocation(), newGhosts, s.getDotLocations(), newMoves, s);
   ...
   * @return
   */
  public static List<GameState> getProjectedStates(GameState s, Move pacManMove) {
     ...
     * @return
     */
  public static List<GameState> projectPacManLocation(GameState s, Move pacManMove, int maxLength) {
    List<GameState> projectedStates = new ArrayList<GameState>();
    GameState curr = getNextGameState(s, pacManMove);
    ...
    while (!isFinal(curr) && legalMoves.size()==1 && length<maxLength) { // repeat this move
      ...
      curr = getNextGameState(curr, pacManMove);
      projectedStates.add(curr);
  ...
  public static boolean isWinning(GameState s) {
    return s.getDotLocations().isEmpty();
  }

  public static boolean isLosing(GameState s) {
    List<Location> ghostLocations = s.getGhostLocations();
  ...
  public static boolean isFinal(GameState s) {
   ...
   */
  private GameState move(Move pacManMove, List<Move> ghostMoves) {
    if (isFinal(getCurrentGameState()))
      throw new RuntimeException(); // no, can't move
    GameState nextGameState = getNextGameState(getCurrentGameState(), pacManMove, ghostMoves);
    // update time, points, and state
    time++;
    if (nextGameState.getDotLocations().size() < currentGameState.getDotLocations().size())
  ...
  public String toString() {
    return toString(getCurrentGameState());
  }
  ...
  public void playOneBoardOneLife(final PacManPlayer pacMan) {
    ...
    int iter = 0;
    while (!Game.isFinal(currentGameState)) {
...
//        t.stop();
  ...
  public int[] playGame(PacManPlayer pacMan, int numLives, LocationSet dotLocations) {
    ...
    currentGameState = new GameState(pacManLaunch, makeGhostLocations(ghostPlayers), dotLocations, previousGhostMoves,
                             null); // no parent, since it's the initial state
    if (eatDot(currentGameState.getDotLocations(), currentGameState.getPacManLocation()))
      points += dotPointsValue;
    while (lives > 0) {
      playOneBoardOneLife(pacMan);
      if (isWinning(currentGameState)) {
        // completely new board
        currentGameState = new GameState(pacManLaunch, makeGhostLocations(ghostPlayers), dotLocations,
      ...
      } else if (isLosing(currentGameState)) {
        // use the old dot locations, but put Pac-Man and the ghosts back in the
        // launch position
        currentGameState = new GameState(pacManLaunch, makeGhostLocations(ghostPlayers),
                                 currentGameState.getDotLocations(), null, null); // no
        // parent,
        // since
        // it's
        // the
