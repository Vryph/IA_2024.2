...
public class PacManGUIGameBoard extends JComponent {
	...
	public void updateDisplay(Game game) {
		if(board == null) {
			...
			oldGameState = game.getCurrentGameState();
			currentGameState = oldGameState;
			ghosts = game.getGhostPlayers();
			repaint();
		} else {
			oldGameState = currentGameState;
			currentGameState = game.getCurrentGameState();
	...
	public void paintComponent(Graphics g) {
		...
		for(int y = Game.yDim-1; y >= 0; y--) {
			for (int x = 0; x < Game.xDim; x++) {
		        ...
		        if(currentGameState.getPacManLocation().equals(l)) {
		        		if(currentGameState.getPacManLocation().getX()
		        				< oldGameState.getPacManLocation().getX())
		        			direction = Move.LEFT;
		        		else if(currentGameState.getPacManLocation().getX()
		        				> oldGameState.getPacManLocation().getX())
		        			direction = Move.RIGHT;
		        		else if(currentGameState.getPacManLocation().getY()
		        				< oldGameState.getPacManLocation().getY())
		        			direction = Move.DOWN;
		        		else if(currentGameState.getPacManLocation().getY()
		        				> oldGameState.getPacManLocation().getY())
		        			direction = Move.UP;
//		        		System.err.println(direction.toString());
		        		drawPacMan(g, (int)Math.round(resX*l.getX()),
		        				   (int)Math.round(resY*(Game.yDim-1-l.getY())),
		        				    direction);
		        } else if (currentGameState.getGhostLocations().contains(l)) {
		        		int index = currentGameState.getGhostLocations().indexOf(l);
		        		drawGhost(g, (int)Math.round(resX*l.getX()),
		        				  (int)Math.round(resY*(Game.yDim-1-l.getY())),
		        				  ghosts.get(index));
		        } else if (currentGameState.getDotLocations().contains(l)) {
